...
public class HqRenderer : MonoBehaviour
{
    ...
    private void DrawObjects()
    {
        ...
        if (drawSprites)
        {
            ...
            for (int n = startPos + DravingDistance; n > startPos; n--)
            {
                ...
                {
                    tokenManager.DrawTokensForSegment(segment, ref line, this);
                }
            }
    ...
    void CalculateProjection()
    {
        ...
        for (int n = startPos + 1; n < startPos + DravingDistance; n++)
        {
            ...
            if ((n / 3) % 2 == 0)
            {
                // Draw one narrow dashed divider between each adjacent pair of lanes.
                // nd is the normalised offset within the road half-width W, so
                // (p.X + nd*p.W) is always inside the road regardless of curve direction.
